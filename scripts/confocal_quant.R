#Plot confocal imaging data for Figure 5
...
path <- "path/to/this/folter/tmorita_2025_data/"
...
dat$driver <- factor(dat$driver, levels=c("brp","ir25a","ir76b","gr4"))
#plot forelegs
foreleg_cranialcaudal_count_plot <- 
  dat %>% 
  filter(legs == 'foreleg') %>% 
  plot_cranialcaudal_count(y=c(0,60))
#plot midlegs
midleg_cranialcaudal_count_plot <-
  dat %>% 
  filter(legs == 'midleg') %>% 
  plot_cranialcaudal_count(y=c(0,60))