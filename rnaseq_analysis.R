#DE analysis using most updated transcript annotation files
library(tidyverse)
library(tximport)
library(DESeq2)
...
f <- paste0(path,"/figure7/AaegLVP_VB58-Jove19.gtf")
txdb <- makeTxDbFromGFF(f,format="gtf")
k <- keys(txdb, keytype = "TXNAME")
tx2gene <- select(txdb, k, "GENEID", "TXNAME")

#load quantified mapped reads
dir <- paste0(path,"/figure7/orco_vs_wt_quants")
...
f <- paste0(path,"/figure7/chemoreceptor_gene_list.tsv")
...
#####################################
f <- paste0(path,"/figure7/AaegLVP_VB58-Jove19_geneTypes.tsv")
gene_types <- 
  read_tsv(f) %>% 
